...
from app.podcastCreator import (merge_text_files,
                                 ...
                                 cosine_similarity, add_intro_music, getScriptfromGemini, getNBAPodcastContent,
                                 generate_revised_script)
import pytest
import builtins
import numpy as np
...
# Helper function to create a fixed datetime for testing
...
@patch('app.podcastCreator.datetime.datetime', _fixed_datetime(datetime.datetime(2022, 3, 2)))
def test_getNBAPodcastContent_success(mock_get):
    # Setup the mock response to simulate successful HTML content retrieval
    ...
    # Assert that the start and end markers are found in the content
...
def test_getScriptfromGemini_success(mock_start_chat):
    ...
    # Mock response setup to simulate the expected interaction
    ...
    # Execute the function with a test topic
    topic = "Artificial Intelligence"
    # Assuming getScriptfromGemini processes but doesn't return the script
    _ = getScriptfromGemini(topic)  # Ignoring the return as the function currently returns None
...
def test_getScriptfromGemini_api_failure(mock_start_chat):
    ...
    # Execute the function and expect an exception or a specific failure response
    topic = "Artificial Intelligence"
    with pytest.raises(Exception) as exc_info:
